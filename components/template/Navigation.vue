<template>
  <header style="z-index:9999;position: relative;" class="flex items-center justify-between flex-wrap bg-white dark:bg-black text-black dark:text-white p-6">
    <div class="flex items-center flex-shrink-0 mr-6">
      <span class="font-semibold text-xl tracking-tight">
        Q|vanderperre
      </span>
    </div>
    <div class="block lg:hidden">
      <button class="flex items-center px-3 py-2 border rounded border-teal-400 hover:text-white hover:border-white">
        <svg class="fill-current h-3 w-3" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          <title>
            Menu
          </title>
          <path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z" />
        </svg>
      </button>
    </div>
    <div class="w-full block flex-grow lg:flex lg:items-center lg:w-auto">
      <nav id="nav" class="text-sm lg:flex-grow">
        <a href="#about" class="block mt-4 lg:inline-block lg:mt-0 hover:text-white mr-4">
          A propos
        </a>
        <a href="#projects" class="block mt-4 lg:inline-block lg:mt-0 hover:text-white mr-4">
          Projets
        </a>
        <a href="#contact" class="block mt-4 lg:inline-block lg:mt-0 hover:text-white">
          Contact
        </a>
      </nav>
      <div class="w-full block flex-grow lg:flex lg:items-center lg:w-auto">
        <lighting-switcher class="inline-block" />
      </div>
    </div>
  </header>
</template>

<script>
import LightingSwitcher from '@/components/template/LightingSwitcher'

export default {
  name: 'Navigation',
  components: {
    LightingSwitcher
  }
}
</script>
